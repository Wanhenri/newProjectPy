...
for ind in range(0,7,1):
    ...
    for previsao in range(24,192,24):
        ...
        StandardDeviation_umidadeGL = StandardDeviation(umidadeGL)
        #print('prev ', prev,'StandardDeviation umidade GL: ', StandardDeviation_umidadeGL)
        StandardDeviation_umidadeNova = StandardDeviation(umidadeNova)
        #print('prev ',prev,'umidade nova: ', StandardDeviation_umidadeNova)
        StandardDeviation_modeloGFS = StandardDeviation(modeloGFS)
        #print('prev ',prev,'StandardDeviation GFS: ', StandardDeviation_modeloGFS)

        lista_previsao.append(previsao)
        lista_umidadeGL.append(StandardDeviation_umidadeGL)
        lista_umidadeNova.append(StandardDeviation_umidadeNova)
        lista_modeloGFS.append(StandardDeviation_modeloGFS)
    ...
    #plt.figtext(.5,.96,'Root Mean Squared Error (StandardDeviation)', fontsize=30, ha='center')
    #plt.figtext(.5,.90,'Janeiro/2014',fontsize=20,ha='center')
    plt.figtext(.5,.96,'20140101 12Z', fontsize=30, ha='center')
    plt.figtext(.4,.90,'Standard Deviation    ' + longName[8:30] + '      modelRef: MERGE' ,fontsize=30,ha='center')
    ...
    title = 'regiao_'+ config['Regiao'][ind] +'_'+ config['Setor'][ind]+'_StandardDeviation_PREC_withMERGE.png'

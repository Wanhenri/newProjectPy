...
def serieTemporal(prev):
  ...
  for ind in range(0,7,1):
    ...
    path_5 = "/dados/dmdpesq/Experimento_umidade_do_solo/GFS/"    
    name_file_5 = 'prev.2014.jan_APCP_12z_'+ prev +'h_interp.nc'

    #'prev.2014.jan_APCP_12z_'+ prev +'h_interp.nc'
    ...
    GFS_new = xr.open_dataset(path_5 + name_file_5)
    p5 = GFS_new.APCP_surface.isel(time=slice(None, 31), lat=slice(latNorte,latSul), lon=slice(lonOeste,lonLest)).mean(dim="lat").mean(dim="lon")
    plt.plot(xTickTime, p5, color='k', label='GFS_new')
    plt.xticks(rotation=45)
    ...
    title = 'previsao_'+ prev +'_regiao_'+ config['Regiao'][ind] +'_'+ config['Setor'][ind]+'_SerieTemporal_GFS_MERGE_BAM_teste2novoGFS.png'
